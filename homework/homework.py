...
def load_data():
    # Base dir: sube un nivel desde homework/
    base_dir = os.path.dirname(os.path.dirname(__file__))
    input_path = os.path.join(base_dir, "files", "input")

    test_path = os.path.join(input_path, "test_data.csv.zip")
    train_path = os.path.join(input_path, "train_data.csv.zip")

    # Validaciones opcionales
    if not os.path.exists(test_path):
        raise FileNotFoundError(f"No se encontró el archivo de test: {test_path}")
    if not os.path.exists(train_path):
        raise FileNotFoundError(f"No se encontró el archivo de train: {train_path}")
